...
def search_tweets():
    ...
    keyword_list=[sys.argv[4]]
    Max_tweet_id=int(sys.argv[5])
    total=0
...
print("検索完了")
print("csv結合")
tempdf=pd.read_csv("../static/csv/temp-{}".format(PATH))
df=pd.concat([df,tempdf])
df.drop_duplicates(keep=False,subset=["ユーザーネーム","テキスト"],inplace=True)
print("分類開始")
test_data=Test_TFIDF(df)
model=load_model("lr")
test_predictions=predict_model(model,data=test_data)
test_predictions[["ユーザーネーム","テキスト","URL","日付"]][test_predictions["Label"]==1].to_csv("../static/csv/{}".format(PATH),index=False)
print("分類終了")